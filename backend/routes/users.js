...
router.put('/:id', [
  ...
  body('designation').optional().trim().isLength({ min: 2, max: 100 }).withMessage('Designation must be between 2 and 100 characters'),
  body('qualification').optional().trim().isLength({ min: 2, max: 200 }).withMessage('Qualification must be between 2 and 200 characters'),
  body('mobileNo').optional().matches(/^[0-9]{10}$/).withMessage('Please provide a valid 10-digit mobile number'),
  body('department').optional().isIn(['Bachelor of Education (BED)', 'Civil Engineering (CIVIL)', 'Computer Science & Engineering (CSE)', 'Artificial Intelligence Data Science & Machine Learning (AIDS & ML)', 'Information Technology & Master of Computer Applications (IT & MCA)', 'Electronics & Communication Engineering (ECE)', 'Electrical & Electronics Engineering (EEE)', 'Mechanical Engineering (MECH)']).withMessage('Invalid department'),
