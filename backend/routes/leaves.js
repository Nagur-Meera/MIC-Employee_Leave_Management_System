...
router.put('/:id/status', [
  protect,
  authorize('admin', 'hod'),
  body('status').isIn(['approved', 'hodApproved', 'rejected']).withMessage('Status must be approved, hodApproved, or rejected'),
  ...
  try {
    ...
    if (req.user.role === 'hod') {
      ...
      // HOD can only process pending leaves
      ...
      // HODs can only set to hodApproved or rejected
      if (status === 'approved') {
        return res.status(400).json({
          success: false,
          message: 'HODs can only set leaves to hodApproved or rejected. Final approval is done by admin'
        });
      }

      if (status === 'hodApproved') {
        // HOD is approving for the first step
        leave.status = 'hodApproved';
        leave.approvalLevel = 'hod';
    ...
    // Admin approval flow
      ...
      // Admin can only approve/reject leaves that have been approved by HOD or reject pending leaves
      if (status === 'approved' && leave.status !== 'hodApproved') {
        return res.status(400).json({
          success: false,
          message: 'Leave application must be approved by HOD first'
        });
      }
      ...
      if (status === 'hodApproved') {
        return res.status(400).json({
          success: false,
          message: 'Admin should provide final approval or rejection'
        });
      }
      
      // Admin can reject at any stage
      if (status === 'rejected' && (leave.status === 'pending' || leave.status === 'hodApproved')) {
        leave.status = 'rejected';
      ...
      else if (status === 'approved' && leave.status === 'hodApproved') {
        leave.status = 'approved';
        leave.approvalLevel = 'admin';
        leave.approvedBy = req.user.id;
        leave.approvedAt = new Date();
        
        // If approved by admin, update user's leave balance
...
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    if (leave.status !== 'pending' && leave.status !== 'hodApproved') {
      return res.status(400).json({
        success: false,
        message: 'Only pending or HOD approved leave applications can be cancelled'
      });
    }
