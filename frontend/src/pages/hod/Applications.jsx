...
const HODApplications = () => {
  ...
  const handleApprove = async (applicationId) => {
    try {
      await api.put(`/leaves/${applicationId}/status`, { status: 'hodApproved' });
      fetchApplications();
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'hodApproved':
        return <span className="badge status-info">HOD Approved</span>;
      case 'approved':
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Review and manage leave applications from {user?.department}</p>
        </div>
                  ...
                  <tr key={application._id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{application.employee.role?.toUpperCase() || 'EMPLOYEE'}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="text-gray-900">{selectedApplication.employee.role.toUpperCase()}</div>
                </div>
                <div>
                  <label className="label">Leave Type</label>
                  <div>{getLeaveTypeBadge(selectedApplication.leaveType)}</div>
                </div>
                <div>
